      ...
      try {
        ...
        if (sorted.length > 0) {
          ...
          if (sorted.length > 0) {
            ...
            // Fetch khusus buat hitung komen si latest
            fetch(`${API_BASE}/forums/${latestId}/comments`)
              .then(res => res.json())
              .then(data => setLatestCommentCount(data.length))
        ...
        {loading ? (
          ...
          <div className="flex justify-center w-full md:max-w-[90%] mx-auto">
            <div
              ...
              //   router.push(`/himtalks/mini-forum/${slug}#comment`);
              // }}
              className="bg-white w-full rounded-2xl shadow-lg p-4 md:p-5 relative border border-gray-100 hover:-translate-y-2 transition duration-300 cursor-pointer selection:bg-darkSage selection:text-white"
            >
              {/* HEADER (Himtalks + Time) */}
              <div className="flex justify-between items-center mb-3 md:mb-5 font-poppins">
                <div className="flex items-center gap-1 md:gap-2 text-[10px] md:text-xs lg:text-sm xl:text-base text-gray-500 font-poppins">
                  <img src="/logo/himtalks-logo.webp" alt="Himtalks" className="w-5 h-5 md:w-6 md:h-6 object-cover rounded-full" />
                  <span className="text-black">Himtalks</span>
                  <span>•</span>
                  <span>{timeAgo(latest.created_at)}</span>
                ...
                <span className="text-[9px] leading-4 md:text-xs 2xl:text-sm border px-2 md:px-3 py-0.5 md:py-1 rounded-full text-gray-500">
                  19.00 - 21.00 WIB
                </span>
              </div>
              ...
              {/* DESC / CONTENT */}
              <p className="font-poppins tracking-tighter text-gray-500 text-[11px] md:text-xs lg:text-sm mb-3 md:mb-4 line-clamp-2 text-ellipsis">
                {latest.content}
              </p>
              ...
              >
                <span className="font-poppins inline-flex items-center gap-1 md:gap-2 bg-darkSage text-white hover:bg-white hover:text-darkSage px-3 py-1 md:px-4 md:py-2 rounded-full text-xs md:text-sm border border-darkSage transition-all duration-500">
                  ...
                  {/* <span>{latest.comment_count} Komentar</span> */}
                  <span>{latestCommentCount} Komentar</span>
                </span>
              </button>
            </div>
          </div>
